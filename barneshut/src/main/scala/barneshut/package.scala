...
package object barneshut {
  ...
  case class Fork(
    ...
    private val quadtreesList = List(nw, ne, sw, se)

    val centerX: Float = (nw.centerX + ne.centerX) / 2f
    val centerY: Float = (nw.centerY + sw.centerY) / 2f
    val size: Float = nw.size + ne.size
    val mass: Float = quadtreesList.foldLeft(0f) {
      _ + _.mass
    }
    val massX: Float = {
      if (mass == 0) centerX
      else quadtreesList.foldLeft(0f) { case (sum, quad) => sum + quad.mass * quad.massX } / mass
    }
    val massY: Float = {
      if (mass == 0) centerY
      else quadtreesList.foldLeft(0f) { case (sum, quad) => sum + quad.mass * quad.massY } / mass
    }
    val total: Int = quadtreesList.foldLeft(0) {
      _ + _.total
    }
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    val (massX, massY) = (
      bodies.foldLeft(0f) { case (sum, b) => sum + b.mass * b.x } / mass,
      bodies.foldLeft(0f) { case (sum, b) => sum + b.mass * b.y } / mass
    )
