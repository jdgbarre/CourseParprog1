...
object HorizontalBoxBlur {
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    ...
    val rowsPerTasks: Int = src.height / numTasks max 1
    val startPoints: Seq[RGBA] = 0 to src.height by rowsPerTasks

    val tasks: Seq[ForkJoinTask[Unit]] = (startPoints zip startPoints.tail) map {
